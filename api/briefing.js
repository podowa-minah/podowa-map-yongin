...
export default async function handler(req, res) {
  ...
  try {
    const {
      ...
      monthly = [],            // 이번 시즌 월별 흐름(지난달 대비) — Tier2 기억
    } = req.body || {};
    ...
    const facts = [
      ...
      monthly.length && `이번 시즌 월별 흐름(지난달 대비):\n${monthly.join('\n')}`,
    ].filter(Boolean).join('\n');
...
2. 오늘 상태를 아래 3개 범주로 기록한다(영농일지에 날짜별로 쌓임). **사실 서술만**. 요청·지시 금지.
   ★ 짧게 — 각 범주는 **전체 한 줄 요약 + 특이하거나 꼭 기억할 예외 1~2개만**.
     예) growth: "세력 3.8로 안정적인 흐름이에요." / pest: "해충 0.3으로 전체적으로 아주 양호해요. 다만 5-9 함부르크에 유충·개미 집단이 새로 보여요."
     멀쩡한 나무를 일일이 나열하지 마라(좌표는 정말 특이한 것만). 잠잠한 나무 목록·줄어든 나무 목록은 적지 마.
...
- **이번 시즌 월별 흐름**이 있으면 큰 변화만 "지난달 대비"로 한 번 짚어라(예: "지난달보다 세력이 올라왔어요", "해충이 지난달부터 오름세"). 단, 준 데이터에 있는 달만. 매달 나열하지 말고 의미 있는 변화 1가지.
- 데이터(숫자·추이·사람 메모·최근 흐름·월별)에 없는 사실을 지어내지 마. 과장 금지.`;
